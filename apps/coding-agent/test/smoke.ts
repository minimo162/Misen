...
async function testUiContract(): Promise<void> {
  ...
  assert.ok(html.includes('<title>Misen</title>'), 'default shell must expose the Misen product title')
  assert.ok(frontend.includes("h('span', null, 'Misen')") && frontend.includes("h('strong', null, 'Misen')"), 'default UI must expose the Misen brand')
  assert.ok(classic.includes('<title>Misen</title>') && classic.includes('</span>Misen<button'), 'classic UI must expose the Misen brand')
...
// Issue #93 C: sub-tests are tiered. "unit" sub-tests use only in-process fakes and finish in a few seconds.
// "integration" sub-tests spawn real child processes (node/powershell) or wait on real loopback servers and
// wall-clock deadlines; `npm run test:integration` runs them. `--tier all` (default) runs everything.
const SMOKE_TESTS: Array<{ name: string; run: () => Promise<void>; tier: 'unit' | 'integration' }> = [
...
function selectedTier(): 'unit' | 'integration' | 'all' {
