...
The deterministic corpus varied workspace root, `reports`, `rates`, `tools`, an empty directory, and a mixed Japanese-name directory containing txt, CSV, xlsx, and Markdown. The converter corpus used different strict JSON, flattened JSON, labeled text, and plain Japanese forms. The adopted Q4_K_M converter run passed 16/16; strict JSON used the schema-validated fast path in 0–1 ms, while malformed/plain positive responses took 2.3–5.2 s on CPU. A separate eight-case negative gate, including explanatory mentions of tool names and an unknown tool, produced zero false-positive tool calls (longest negative conversion: 15.0 s).
