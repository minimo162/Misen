...
For repeatable preparation-PC performance evidence, start the agent on port 3951 with a disposable workspace and run `npm run flex:measure:copilot -- http://127.0.0.1:3951 .tmp/flex-copilot-performance .tmp/flex-performance/result.json`. The driver exercises list, read, search, new-file write, and open with fresh sessions, auto-approves only its disposable write/open fixture, and records per-phase timings. The accepted five-task run was 5/5 at 19.952–36.299 seconds; converter and Copilot generation were the dominant intervals. See `FLEX-VALIDATION.md` for the one-change-at-a-time comparison and rejected experiments.
