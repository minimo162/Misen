...
function normalizeRunCommand(command) {
  ...
  if (isReadXlsx) {
    ...
    const reportPaths = normalized.length > 1 ? normalized.filter((word) => !(import_node_path.default.win32.dirname(word) === "." && import_node_path.default.win32.basename(word).toLowerCase() === "\u96C6\u8A08\u53F0\u5E33.xlsx")) : normalized;
    ...
    if (reportPaths.length === 1) {
      ...
      const directories = new Set(reportPaths.map((word) => import_node_path.default.win32.dirname(word).toLowerCase()));
      if (directories.size !== 1 || reportPaths.some((word) => import_node_path.default.win32.extname(word).toLowerCase() !== ".xlsx")) {
...
function realPathWithMissingTail(abs) {
  ...
  while (!import_node_fs.default.existsSync(cursor)) {
    ...
    tail.unshift(import_node_path.default.basename(cursor));
    cursor = parent;
  }
  const real = import_node_fs.default.realpathSync.native(cursor);
  return import_node_path.default.resolve(real, ...tail);
}
...
function resolveInWorkspace(p, ctx) {
  ...
  const workspaceAbs = import_node_path.default.resolve(ctx.workspace);
  const abs = import_node_path.default.isAbsolute(p) ? import_node_path.default.normalize(p) : import_node_path.default.resolve(workspaceAbs, p);
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const addFile = (abs, displayPath) => {
        ...
        const relative = (displayPath ?? import_node_path.default.relative(ctx.workspace, checked)).replaceAll("\\", "/");
      ...
      if (patterns.length > 0) {
        ...
        await walk(ctx.workspace, (file) => {
          const relative = import_node_path.default.relative(ctx.workspace, file).replaceAll("\\", "/");
...
`;
        ...
        if (import_node_path.default.extname(file.relative).toLowerCase() === ".xlsx") {
...
`;
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const result = formatFileChangeResult("\u66F8\u304D\u8FBC\u307F", import_node_path.default.relative(ctx.workspace, abs), before, readBack, 1, existedBefore);
      return `${result}
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      if (next === src) {
        ...
        return formatFileChangeResult("\u7DE8\u96C6", import_node_path.default.relative(ctx.workspace, abs), src, src, count, true);
      }
      ...
      return formatFileChangeResult("\u7DE8\u96C6", import_node_path.default.relative(ctx.workspace, abs), src, readBack, count, true);
    }
  },
  {
    name: "search_files",
...
function toolRequestKey(name, args) {
  ...
  if (typeof copy.path === "string") copy.path = import_node_path2.default.normalize(copy.path).replaceAll("\\", "/");
  if (bare === "list_files") {
...
async function bootstrapWorkspaceEvidence(ctx, io) {
...
var CopilotEdgeClient = class {
  ...
  chooseEdgeProfile() {
    ...
    const stable = import_node_path3.default.join(root, suffix === "default" ? "edge-profile" : "edge-profile-" + suffix);
    if (!profileIsInUse(stable)) return stable;
    return import_node_fs2.default.mkdtempSync(import_node_path3.default.join(root, "edge-profile-" + suffix + "-session-"));
  }
  async ensureEdge() {
...
async function testWeather() {
  import_node_assert.default.strictEqual(weatherCodeLabel(0).emoji, "\u2600\uFE0F");
  import_node_assert.default.strictEqual(weatherCodeLabel(2).emoji, "\u26C5");
  import_node_assert.default.strictEqual(weatherCodeLabel(3).label, "\u66C7\u308A");
  import_node_assert.default.strictEqual(weatherCodeLabel(45).emoji, "\u{1F32B}\uFE0F");
  import_node_assert.default.strictEqual(weatherCodeLabel(61).emoji, "\u{1F327}\uFE0F");
  const calls = [];
  const fetcher = async (input) => {
    ...
    return new Response(JSON.stringify({
      ...
      daily: { time: ["2026-08-24"], temperature_2m_max: [33], temperature_2m_min: [24.8], weather_code: [1] }
    }), { status: 200 });
  };
  const result = await getWeather("\u5E83\u5CF6\u5E02", void 0, fetcher);
  import_node_assert.default.strictEqual(calls.length, 2);
  import_node_assert.default.ok(calls[0].includes("name=%E5%BA%83%E5%B3%B6%E5%B8%82"));
  import_node_assert.default.ok(result.includes("\u5E83\u5CF6\u5E02\uFF08\u5E83\u5CF6\u770C\u30FB\u65E5\u672C\uFF09"));
  import_node_assert.default.ok(result.includes("27.8\xB0C"));
  import_node_assert.default.ok(result.includes("\u2600\uFE0F \u5FEB\u6674"));
  import_node_assert.default.ok(result.includes("\u6700\u9AD8 33.0\xB0C / \u6700\u4F4E 24.8\xB0C"));
  import_node_assert.default.ok(result.includes("Open-Meteo"));
  let missing = false;
  try {
    await getWeather("", void 0, fetcher);
  } catch (err) {
    missing = String(err.message).includes("\u5730\u57DF\u540D\u304C\u5FC5\u8981");
  }
  import_node_assert.default.ok(missing, "weather should reject missing location");
...
async function testTools() {
  ...
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(root, "batch", "empty.txt"), Buffer.alloc(0));
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(root, "batch", "ledger.xlsx"), Buffer.from("not-read-as-text"));
  ...
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(root, "batch", "large-a.txt"), "A".repeat(9e4));
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(root, "batch", "large-b.txt"), "B".repeat(100));
  ...
  import_node_assert.default.strictEqual(await get("read_files").run({ pattern: "missing/**/*.txt" }, ctx), "(\u8A72\u5F53\u306A\u3057)");
  ...
  try {
    ...
    wttrBlocked = String(err.message).includes("get_weather");
  }
  import_node_assert.default.ok(wttrBlocked, "wttr.in should be routed to get_weather");
  ...
  import_node_assert.default.ok(createdMeta && createdMeta.existedBefore === false);
  ...
  import_node_assert.default.ok(!import_node_fs3.default.existsSync(import_node_path4.default.join(root, "journal", "new.txt")), "new files must be deleted on rollback");
...
async function testAgentLoop() {
  ...
  await withServer(
    ...
    async (cfg) => {
      ...
      import_node_assert.default.ok(import_node_fs3.default.readFileSync(import_node_path4.default.join(root, "hello.txt"), "utf8").includes("hi from mock"));
    }
  );
  import_node_fs3.default.rmSync(root, { recursive: true, force: true });
...
async function testCopilotChoosesFirstAction() {
  const answerRoot = import_node_fs3.default.mkdtempSync(import_node_path4.default.join(import_node_os.default.tmpdir(), "ca-smoke-"));
  ...
  import_node_assert.default.ok(!safeBackend.prompts[0].includes("\u4EFB\u610F\u306Ehost\u30B3\u30DE\u30F3\u30C9\u5B9F\u884C\u304C\u8A31\u53EF"), "safe prompt must not claim arbitrary command permission");
  import_node_assert.default.ok(safeBackend.prompts[0].includes("\u65E2\u5B58\u306E\u30EF\u30FC\u30AF\u30B9\u30DA\u30FC\u30B9\u5185\u901A\u5E38\u30D5\u30A1\u30A4\u30EB1\u4EF6"));
  import_node_assert.default.ok(safeBackend.prompts[0].includes("answer\u3067\u5229\u7528\u8005\u3078\u8A31\u53EF\u3092\u5C0B\u306D\u305A"));
  const safeTools = openAITools({ allowArbitraryCommands: true, safeCommandOnly: true });
  import_node_assert.default.ok(!safeTools.some((tool) => tool.function.name === "host.get_weather"));
  const safeStart = safeTools.find((tool) => tool.function.name === "host.start_process");
  import_node_assert.default.ok(safeStart && !("url" in (safeStart.function.parameters.properties ?? {})), "safe start_process schema must omit url");
  import_node_assert.default.ok(safeTools.find((tool) => tool.function.name === "host.run_command")?.function.description.includes("\u4EFB\u610F\u30B7\u30A7\u30EB"));
  ...
  import_node_fs3.default.rmSync(toolRoot, { recursive: true, force: true });
  const repeatRoot = import_node_fs3.default.mkdtempSync(import_node_path4.default.join(import_node_os.default.tmpdir(), "ca-smoke-"));
  ...
  import_node_assert.default.strictEqual(repeat.aborted, true);
  ...
  import_node_fs3.default.rmSync(repeatRoot, { recursive: true, force: true });
...
async function testModeBoundaries() {
  ...
  import_node_assert.default.deepStrictEqual({ hostTools: workPolicy.hostTools, nativeSearch: workPolicy.nativeSearch, maxHostExecutions: workPolicy.maxHostExecutions }, { hostTools: "all", nativeSearch: false, maxHostExecutions: 8 });
  import_node_assert.default.deepStrictEqual({ hostTools: researchPolicy.hostTools, nativeSearch: researchPolicy.nativeSearch, nativeOffice: researchPolicy.nativeOffice }, { hostTools: "none", nativeSearch: true, nativeOffice: false });
  const researchBackend = new FakeBackend(["\u691C\u7D22\u7D50\u679C: https://example.com\uFF08\u53D6\u5F97 2026-08-24T09:00+09:00\uFF09"]);
  const research = await runAgentTurn({
    cfg: { baseURL: "", model: "", provider: "copilot-edge", turnMode: "research", copilot: { agentMode: false } },
    messages: [],
    userInput: "\u6700\u65B0\u60C5\u5831\u3092\u8ABF\u3079\u3066",
    ctx: makeCtx(import_node_os.default.tmpdir(), false),
    io: ioStub(true),
  ...
  import_node_assert.default.strictEqual(research.aborted, false);
  import_node_assert.default.ok(research.research);
  import_node_assert.default.strictEqual(research.research?.sources.length, 1);
  import_node_assert.default.strictEqual(research.research?.sources[0].url, "https://example.com");
  import_node_assert.default.ok(research.research?.contentHash);
  import_node_assert.default.ok(researchBackend.prompts[0].includes("Copilot\u5185\u8535\u306E\u691C\u7D22"));
  import_node_assert.default.ok(!researchBackend.prompts[0].includes("host.get_weather"));
  import_node_assert.default.ok(!researchBackend.prompts[0].includes("TOOL_RESULT (\u7B2C0\u30BF\u30FC\u30F3\u81EA\u52D5\u5B9F\u884C"));
  const sourceRequired = await runAgentTurn({
    ...
    ctx: makeCtx(import_node_os.default.tmpdir(), false),
    io: ioStub(true),
    backend: new FakeBackend(["\u691C\u7D22\u7D50\u679C\u306F\u3042\u308A\u307E\u305B\u3093"])
  });
  import_node_assert.default.strictEqual(sourceRequired.aborted, true);
  import_node_assert.default.ok(sourceRequired.messages.at(-1)?.content?.includes("\u51FA\u5178URL"));
  const chatBackend = new FakeBackend(["\u3053\u3093\u306B\u3061\u306F"]);
  const chat2 = await runAgentTurn({
    cfg: { baseURL: "", model: "", provider: "copilot-edge", turnMode: "chat", copilot: { agentMode: false } },
    messages: [],
    userInput: "\u3053\u3093\u306B\u3061\u306F",
    ctx: makeCtx(import_node_os.default.tmpdir(), false),
    io: ioStub(true),
  ...
  import_node_assert.default.strictEqual(chat2.reply, "\u3053\u3093\u306B\u3061\u306F");
  import_node_assert.default.ok(!chatBackend.prompts[0].includes("host."));
  import_node_assert.default.ok(!chatBackend.prompts[0].includes("TOOL_RESULT (\u7B2C0\u30BF\u30FC\u30F3\u81EA\u52D5\u5B9F\u884C"));
  ...
  const native = await runAgentTurn({
    cfg: { baseURL: "", model: "", provider: "copilot-edge", turnMode: "work", copilot: { agentMode: true } },
    messages: [],
    userInput: "\u691C\u7D22\u3057\u3066\u7DE8\u96C6\u3057\u3066",
    ctx: makeCtx(import_node_os.default.tmpdir(), false),
    io: { ...ioStub(true), event: (event) => nativeEvents.push(event.type) },
    backend: nativeBackend
  });
  import_node_assert.default.strictEqual(native.aborted, true);
  import_node_assert.default.ok(nativeEvents.includes("copilot.native.observed"));
  import_node_assert.default.strictEqual(nativeBackend.calls, 2);
  import_node_assert.default.ok(!openAITools().some((entry) => entry.function.name === "host.run_command"));
  import_node_assert.default.ok(openAITools({ allowArbitraryCommands: true }).some((entry) => entry.function.name === "host.run_command"));
  ...
  const commandDisabled = await runAgentTurn({
    ...
    ctx: makeCtx(import_node_os.default.tmpdir(), false),
    io: ioStub(true),
    backend: commandDisabledBackend
  });
  ...
  const approval = await runAgentTurn({
    ...
    userInput: "\u66F8\u3044\u3066",
    ctx: makeCtx(root),
    io: { ...ioStub(false), event: (event) => approvalEvents.push(event.type) },
  ...
  import_node_assert.default.ok(!import_node_fs3.default.existsSync(import_node_path4.default.join(root, "denied.txt")));
  import_node_fs3.default.rmSync(root, { recursive: true, force: true });
...
async function testProtocolParsing() {
  ...
  import_node_assert.default.ok(fenced && fenced.tool === "host.read_file" && fenced.args && fenced.args.path === "a.txt");
  const prose = extractJsonReply('\u524D\u7F6E\u304D\n{"answer":"\u6CE2\u62EC\u5F27 } \u3092\u542B\u3080\u56DE\u7B54"}\nAGENT_END');
  import_node_assert.default.strictEqual(prose, null);
  const toolNoArgs = extractJsonReply('{"tool":"host.list_files"}');
  import_node_assert.default.ok(toolNoArgs && toolNoArgs.tool === "host.list_files" && toolNoArgs.args);
  const flattened = extractJsonReply('{"tool":"read_files","pattern":"reports/*","AGENT_END":true}');
  import_node_assert.default.deepStrictEqual(flattened, { tool: "read_files", args: { pattern: "reports/*" } });
  const repairedWrite = extractJsonReply('{"tool":"write_file","path":"quote.txt","content":"He said "hello" today","AGENT_END":true}');
  import_node_assert.default.deepStrictEqual(repairedWrite, { tool: "write_file", args: { path: "quote.txt", content: 'He said "hello" today' } });
  ...
  import_node_assert.default.deepStrictEqual(repairedNestedWrite, { tool: "write_file", args: { path: "work/extracted.json", content: nestedJsonContent } });
  import_node_assert.default.strictEqual(extractJsonReply(`\u524D\u7F6E\u304D
${nestedWriteRaw}`), null);
  import_node_assert.default.strictEqual(extractJsonReply(`${nestedWriteRaw}
{"answer":"\u4F59\u5206"}`), null);
  ...
  import_node_assert.default.strictEqual(extractJsonReply('{"tool":"read_files","args":{},"pattern":"reports/*"}'), null);
  import_node_assert.default.strictEqual(extractJsonReply('{"tool":"read_files","AGENT_END":false}'), null);
  import_node_assert.default.strictEqual(extractJsonReply('{"answer":1}'), null);
  import_node_assert.default.strictEqual(extractJsonReply("null"), null);
  import_node_assert.default.strictEqual(extractJsonReply('{"tool":"host.write_file","args":{"path":"x","content":"unterminated}}'), null);
...
async function testCopilotResponseCompletion() {
  const empty = () => ({ stableText: null, stableSinceMs: null });
  ...
  import_node_assert.default.strictEqual(result.ready, false, "same-length content replacement must reset response stability");
  import_node_assert.default.strictEqual(result.state.stableSinceMs, 2300);
  result = updateResponseCompletionState(result.state, {
    observedAtMs: 2400,
    text: "c".repeat(7e3),
    generating: false,
  ...
  try {
    ...
    import_node_assert.default.strictEqual(directDomResponse.answer, "complete response");
    import_node_assert.default.strictEqual(waitBoundaryFinalized, false, "stable visible DOM response must not perform a clipboard round trip");
    boundaryNow = 3e3;
    ...
    const emptyDom = emptyDomClient;
    let emptyPolls = 0;
    emptyDom.readScreenState = async () => {
      emptyPolls++;
      boundaryNow = emptyPolls === 1 ? 3e3 : 4100;
    ...
    import_node_assert.default.strictEqual(clipboardFallback.answer, "clipboard fallback response");
  } finally {
  ...
  orderInternal.waitResponse = async (baseline) => {
    ...
    return { answer: "done", generationWaitMs: 1, completionRetrievalMs: 2 };
  };
...
async function testCopilotLoop() {
  ...
  import_node_assert.default.strictEqual(result.aborted, false);
  import_node_assert.default.strictEqual(import_node_fs3.default.readFileSync(import_node_path4.default.join(root, "work", "extracted.json"), "utf8"), extractedJson);
...
async function testCopilotToolResultBudgets() {
  const readRoot = import_node_fs3.default.mkdtempSync(import_node_path4.default.join(import_node_os.default.tmpdir(), "ca-smoke-"));
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(readRoot, "a-long.txt"), `${"x".repeat(3500)}
LATE_TEXT_MARKER`);
  import_node_fs3.default.writeFileSync(import_node_path4.default.join(readRoot, "z-final.xlsx"), Buffer.from("xlsx-placeholder"));
  ...
  import_node_assert.default.strictEqual(readResult.reply, "read complete");
  ...
  import_node_fs3.default.rmSync(readRoot, { recursive: true, force: true });
  const commandRoot = import_node_fs3.default.mkdtempSync(import_node_path4.default.join(import_node_os.default.tmpdir(), "ca-smoke-"));
  const emitterPath = import_node_path4.default.join(commandRoot, "emit-long.cjs");
  import_node_fs3.default.writeFileSync(emitterPath, "process.stdout.write('A'.repeat(3500) + '\\nlast.xlsx FINAL_WORKBOOK_MARKER')");
...
async function testCopilotFenceMode() {
  ...
  import_node_assert.default.ok(import_node_fs3.default.readFileSync(import_node_path4.default.join(root, "fence.html"), "utf8").includes("<p>fence ok</p>"));
  import_node_fs3.default.rmSync(root, { recursive: true, force: true });
...
async function testLocalResponseConverter() {
  ...
  try {
    import_node_assert.default.strictEqual(await convertCopilotResponse(settings, "raw", []), '{"answer":"raw"}');
    import_node_assert.default.strictEqual(requestCount, 0, "plain answers must stay on deterministic layer 1");
    import_node_assert.default.strictEqual(await convertCopilotResponse(settings, "write_file \u306E\u5224\u65AD\u60C5\u5831\u304C\u4E0D\u8DB3", [
    ...
    import_node_assert.default.strictEqual(deterministicSearch, '{"tool":"host.search_files","args":{"query":"\u9752"}}');
    import_node_assert.default.strictEqual(requestCount, 1, "explicit tool text must stay on deterministic layer 1");
    ...
    const fallback = await runAgentTurn({ cfg: { baseURL: "", model: "", provider: "copilot-edge", copilot: { agentMode: true }, localResponseConverter: settings }, messages: [], userInput: "\u7B54\u3048\u3066", ctx: makeCtx(import_node_os.default.tmpdir(), false), io: ioStub(true), backend: new FakeBackend(["write_file \u306E\u5224\u65AD\u60C5\u5831\u304C\u4E0D\u8DB3", '{"answer":"fallback raw"}\nAGENT_END']) });
  ...
  await import_node_assert.default.rejects(() => convertCopilotResponse({ enabled: true, baseURL: "http://example.com/v1" }, ambiguousWrite, writeTool), /loopback/u);
  await import_node_assert.default.rejects(
...
{'tool':'write_file','args':{'path':'\u30E1\u30E2.txt','content':'\u78BA\u8A8D'}}`,
  ...
  try {
    ...
    import_node_assert.default.strictEqual(impossible.toolCalls, void 0);
    const informational = interpretBridgeResponse("write_file \u306F\u30D5\u30A1\u30A4\u30EB\u3092\u66F8\u304F\u30C4\u30FC\u30EB\u3067\u3059\u3002", tools);
    import_node_assert.default.strictEqual(informational.toolCalls, void 0);
