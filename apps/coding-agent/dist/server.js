...
async function bootstrapWorkspaceEvidence(ctx2, io) {
  const tool = "host.list_files";
  const callId = "host-bootstrap-0";
  const def = findHostTool(tool);
  if (!def) {
    return ["TOOL_RESULT (\u7B2C0\u30BF\u30FC\u30F3\u81EA\u52D5\u5B9F\u884C)", formatHostResult(tool, "[tool error] list_files\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093", null, "failed", callId, ctx2.runId)].join("\n");
  }
  ...
  return [
    ...
    formatHostResult(tool, output, null, failed ? "failed" : "succeeded", callId, ctx2.runId)
  ].join("\n");
}
...
async function runCopilotTurn(opts) {
  ...
  steps.push(await bootstrapWorkspaceEvidence(ctx2, io));
...
async function runOpenAITurn(opts) {
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    let assistant;
    try {
      assistant = await chat(cfg2, messages, openAITools({ allowArbitraryCommands: policy.allowArbitraryCommands, safeCommandOnly: ctx2.safeCommandOnly }), io.signal);
    } catch (err) {
      const msg = err.message;
      io.print(`[error] ${msg}`);
      return { reply: "", messages, aborted: true };
...
var server = import_node_http3.default.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url.pathname === "/api/turn") {
    ...
    try {
      ...
      requestedSessionId = typeof b.sessionId === "string" ? b.sessionId : void 0;
    } catch {
    }
    if (!input) {
      json(res, 400, { error: "message \u304C\u7A7A\u3067\u3059" });
