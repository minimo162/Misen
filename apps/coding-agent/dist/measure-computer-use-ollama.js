...
// test/measure-computer-use-ollama.ts
...
function assertRunCommandPolicy(command, ctx) {
  ...
  const process3 = operations.find((word) => PROCESS_SERVICE_OPERATIONS.has(word));
  if (process3) throw new Error(`run_command\u62D2\u5426: \u30D7\u30ED\u30BB\u30B9\u30FB\u30B5\u30FC\u30D3\u30B9\u64CD\u4F5C ${process3} \u306F\u8A31\u53EF\u3055\u308C\u3066\u3044\u307E\u305B\u3093`);
    ...
    async run(args, ctx) {
      ...
      if (patterns.length > 0) {
        ...
        await walk(ctx.workspace, (file) => {
          const relative = import_node_path2.default.relative(ctx.workspace, file).replaceAll("\\", "/");
...
\u30B5\u30A4\u30BA: ${Buffer.byteLength(readBack)} bytes`;
  ...
  },
  {
    ...
    async run(args, ctx) {
      ...
      const process3 = startManagedProcess(command, ctx.workspace, args.label ? String(args.label) : void 0, args.url ? String(args.url) : void 0);
      return JSON.stringify(process3);
    }
  },
  {
    name: "list_processes",
...
var CdpConnection = class _CdpConnection {
  constructor(socket) {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (response.error !== void 0) call.reject(new Error(`CDP error: ${JSON.stringify(response.error).slice(0, 300)}`));
  ...
  static async connect(url, timeoutMs = 1e4) {
    ...
    return new _CdpConnection(socket);
  }
  method(name, params = {}, timeoutMs = 15e3) {
...
function edgeExecutable() {
  const candidates = [
    process.env["ProgramFiles(x86)"] && import_node_path5.default.join(process.env["ProgramFiles(x86)"], "Microsoft", "Edge", "Application", "msedge.exe"),
    process.env.ProgramFiles && import_node_path5.default.join(process.env.ProgramFiles, "Microsoft", "Edge", "Application", "msedge.exe"),
    process.env.LOCALAPPDATA && import_node_path5.default.join(process.env.LOCALAPPDATA, "Microsoft", "Edge", "Application", "msedge.exe")
...
async function waitForDevTools(port, timeoutMs = 15e3) {
...
async function createPageTarget(port) {
  ...
  const response = await fetch(`http://127.0.0.1:${port}/json`, { signal: AbortSignal.timeout(5e3) });
...
async function waitForDocumentReady(cdp, timeoutMs = 1e4) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const result = await cdp.method("Runtime.evaluate", {
...
var HeadedComputerUseSurface = class _HeadedComputerUseSurface {
  constructor(screen, cdp, edge, profileDirectory, port, targetId, width, height) {
    this.screen = screen;
    this.cdp = cdp;
    this.edge = edge;
  ...
  static async launch(screen, options = {}) {
    ...
    const profileDirectory = await import_promises2.default.mkdtemp(import_node_path5.default.join(import_node_os.default.tmpdir(), "misen-issue53-edge-"));
    const edge = (0, import_node_child_process3.spawn)(edgeExecutable(), [
    ...
    try {
      ...
      const surface = new _HeadedComputerUseSurface(screen, cdp, edge, profileDirectory, port, target.id, width, height);
      await surface.renderCurrentState();
      return surface;
    } catch (error) {
      edge.kill();
      await import_promises2.default.rm(profileDirectory, { recursive: true, force: true }).catch(() => void 0);
      throw error;
    }
  }
  ...
  async capture() {
    ...
    const response = await this.cdp.method("Page.captureScreenshot", {
      format: "png",
      fromSurface: true,
      captureBeyondViewport: false
    });
    if (typeof response.data !== "string" || response.data.length === 0) throw new Error("CDP screenshot payload was empty");
    const bytes2 = new Uint8Array(Buffer.from(response.data, "base64"));
    if (bytes2.byteLength < 8 || !Buffer.from(bytes2.subarray(1, 4)).equals(Buffer.from("PNG", "ascii"))) {
  ...
  async close() {
    try {
      await this.cdp.method("Page.close", {}, 2e3);
    } catch {
    }
    this.cdp.close();
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        await import_promises2.default.rm(this.profileDirectory, { recursive: true, force: true });
        break;
      } catch {
        await new Promise((resolve) => setTimeout(resolve, 100));
      }
...
function parseArgs(args) {
  ...
  return {
    seed,
    budget,
    model: read("--model") ?? import_node_process.default.env.OLLAMA_MODEL ?? "ornith-1.5:9b",
    baseURL: read("--base-url") ?? "http://127.0.0.1:11434/v1",
    outputDirectory: import_node_path6.default.resolve(read("--output") ?? import_node_path6.default.join(".tmp", "computer-use-live")),
    captureOnly: args.includes("--capture-only"),
    omitScreenshot: args.includes("--omit-screenshot")
  };
}
function sha2562(bytes2) {
  return import_node_crypto5.default.createHash("sha256").update(bytes2).digest("hex");
}
function usageFrom(events) {
  return events.filter((event) => event.type === "model.decision").map((event) => event.metadata?.usage ?? null);
}
...
async function saveScreenshot(directory, label, screenshot) {
  await import_promises3.default.mkdir(directory, { recursive: true });
  const output = import_node_path6.default.join(directory, `${label}.png`);
  await import_promises3.default.writeFile(output, screenshot.bytes);
  return output;
}
async function main() {
  const cli = parseArgs(import_node_process.default.argv.slice(2));
  ...
  try {
    ...
    const runDirectory = import_node_path6.default.join(cli.outputDirectory, `seed-${cli.seed}-budget-${dimensions.maxVisualTokens}${cli.omitScreenshot ? "-omitted" : ""}`);
    ...
    if (cli.captureOnly) {
      console.log(JSON.stringify({
        ...
        budget: dimensions,
        path: initialPath,
        screenshot: { bytes: initial.bytes.byteLength, sha256: sha2562(initial.bytes), width: initial.width, height: initial.height }
      }));
      return;
    }
    const io = {
    ...
    const ctx = { workspace: import_node_process.default.cwd(), restrictToWorkspace: true, safeCommandOnly: true, runId: `issue53-${cli.seed}-${dimensions.maxVisualTokens}` };
    const userContent = cli.omitScreenshot ? COMPUTER_USE_DEMO_PROMPT : [
      { type: "text", text: COMPUTER_USE_DEMO_PROMPT },
      { type: "image", mediaType: initial.mediaType, image: initial.bytes, estimatedVisualTokens: dimensions.estimatedVisualTokens }
    ];
    let result;
    try {
      result = await runAgentTurnV2({
        cfg,
        messages: [],
        userInput: COMPUTER_USE_DEMO_PROMPT,
        userContent,
        ctx,
        io,
        visualTokenBudget: dimensions.maxVisualTokens,
      ...
      console.log(`COMPUTER_USE_LIVE_SUMMARY ${JSON.stringify({
        mode: "negative-no-screenshot",
        seed: cli.seed,
        model: cli.model,
        elapsedMs: Date.now() - startedAt,
        failClosed,
        modelCalls: events.filter((event) => event.type === "model.decision").length,
        approvals: approvals.length,
        screenChanged: screen.isOpened,
        error: error instanceof Error ? error.message : String(error)
      })}`);
      if (!failClosed || events.some((event) => event.type === "model.decision") || approvals.length > 0 || screen.isOpened) import_node_process.default.exitCode = 2;
      return;
    }
    const toolCalls = toolCallsFrom(result.messages);
    const call = toolCalls[0];
    const selected = call?.arguments && typeof call.arguments === "object" && !Array.isArray(call.arguments) ? call.arguments.company_id : void 0;
    ...
    const summary = {
      ...
      initial: { bytes: initial.bytes.byteLength, sha256: sha2562(initial.bytes), width: initial.width, height: initial.height },
      post: post ? { bytes: post.bytes.byteLength, sha256: sha2562(post.bytes), width: post.width, height: post.height } : null,
      toolCalls,
      correctTarget: selected === screen.targetCompanyId,
      screenChanged: screen.isOpened && post !== void 0 && sha2562(initial.bytes) !== sha2562(post.bytes),
      approvals: approvals.length,
    ...
    if (cli.omitScreenshot) {
      if (screen.isOpened || approvals.length > 0) import_node_process.default.exitCode = 2;
