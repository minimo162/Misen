...
var require_lib = __commonJS({
  "vendor/npm/node_modules/iconv-lite/lib/index.js"(exports2, module2) {
    ...
    iconv2.getEncoder = function getEncoder(encoding, options) {
      var codec = iconv2.getCodec(encoding), encoder3 = new codec.encoder(options, codec);
...
`;
  const suffix = `
${HOST_RESULT_END}`;
  if (!value.startsWith(prefix) || !value.endsWith(suffix)) return void 0;
...
var execFileAsync = import_node_util.default.promisify(import_node_child_process.execFile);
...
function testUtf8AndJsonCaps() {
  const utf8 = "\u524D\u7F6E\u304D\u{1F600}".repeat(100);
  const capped = capToolResultForModel(utf8, { maxBytes: 256, chunkBytes: 128 });
  import_strict.default.equal(capped.truncated, true);
  import_strict.default.ok(byteLength(capped.content) <= 256);
  import_strict.default.match(capped.content, /tool-result-truncated/u);
  import_strict.default.ok(!capped.content.includes("\uFFFD"));
  import_strict.default.ok(capped.chunks.length > 1);
  import_strict.default.ok(capped.chunks.every((chunk) => byteLength(chunk) <= 128 && !chunk.includes("\uFFFD")));
  ...
  import_strict.default.equal(json.format, "json");
  import_strict.default.ok(byteLength(json.content) <= 512);
  const envelope = JSON.parse(json.content);
  import_strict.default.equal(envelope.summary?.facts?.status, "succeeded");
  import_strict.default.equal(envelope.summary?.facts?.approved, true);
  import_strict.default.equal(envelope.summary?.facts?.precondition, "unchanged");
  import_strict.default.equal(envelope.summary?.facts?.secretToken, void 0);
...
function testAtomicProjectionAndCompleteHistory() {
  const history = [
    ...
    { role: "tool", content: [{ type: "tool-result", toolCallId: "current", toolName: "read_file", output: { type: "text", value: "x".repeat(8e3) } }] },
  ...
  import_strict.default.deepEqual(history, original, "complete session history must not be mutated");
  const serialized = JSON.stringify(result.messages);
  import_strict.default.ok(serialized.includes("current request"));
  import_strict.default.ok(serialized.includes("current"));
  import_strict.default.ok(serialized.includes("old result"), "complete prior tool facts must be retained without a semantic stale proof");
  import_strict.default.ok(!serialized.includes("orphan"));
  import_strict.default.ok(!serialized.includes("private chain"));
  import_strict.default.ok(!serialized.includes("base64-bytes"));
  import_strict.default.match(serialized, /tool-result-truncated/u);
  import_strict.default.equal(result.telemetry.dropped.toolPairs, 0);
  import_strict.default.equal(result.telemetry.dropped.orphanToolCalls, 0);
  import_strict.default.equal(result.telemetry.dropped.orphanToolResults, 1);
  import_strict.default.equal(result.telemetry.dropped.reasoning, 1);
  import_strict.default.equal(result.telemetry.dropped.images, 1);
  const calls = [...serialized.matchAll(/"toolCallId":"([^"]+)"/gu)].map((match) => match[1]);
  import_strict.default.deepEqual(calls, ["old", "old", "current", "current"], "every retained tool call/result must remain paired");
...
function testAllToolPairsAreRetained() {
  const history = [{ role: "user", content: "combine observations" }];
  ...
  const serialized = JSON.stringify(result.messages);
  import_strict.default.ok(serialized.includes("call-1"), "initial task observation must be retained");
  import_strict.default.ok(serialized.includes("call-2"), "middle prerequisite facts must not be dropped by position");
  import_strict.default.ok(serialized.includes("call-3") && serialized.includes("call-4"), "latest pairs must be retained");
  import_strict.default.equal(result.telemetry.dropped.toolPairs, 0);
}
function testHostResultEnvelopeRemainsParseableAfterCap() {
  const wrapped = formatHostResult("host.read_file", "\u7D50\u679C\u{1F600}".repeat(4e3), { path: "report.txt" }, "succeeded", "call-large", "run-large");
  ...
  import_strict.default.ok(output.startsWith(begin) && output.endsWith(end));
  import_strict.default.doesNotThrow(() => JSON.parse(output.slice(begin.length, -end.length)));
  import_strict.default.ok(byteLength(output) <= 4096);
  import_strict.default.match(output, /tool-result-truncated/u);
}
function testMixedOrphanPartsAreRemovedById() {
  const result = buildModelWorkingContext([
  ...
  import_strict.default.deepEqual(ids, ["matched-call", "matched-call", "matched-result", "matched-result"]);
  import_strict.default.equal(result.telemetry.dropped.orphanToolCalls, 1);
  import_strict.default.equal(result.telemetry.dropped.orphanToolResults, 1);
  import_strict.default.ok(!serialized.includes("must not leak into protocol"));
}
testUtf8AndJsonCaps();
