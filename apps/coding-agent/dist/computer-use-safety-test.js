...
function validRecord(value) {
  ...
  const record = value;
  return record.schema_version === 1 && typeof record.event_id === "string" && typeof record.timestamp === "string" && typeof record.session_id === "string" && typeof record.run_id === "string" && (record.call_id === null || typeof record.call_id === "string") && typeof record.tool_name === "string" && !!record.arguments && typeof record.arguments.summary === "string" && typeof record.arguments.sha256 === "string" && !!record.permission && (record.permission.decision === "allow" || record.permission.decision === "ask" || record.permission.decision === "deny") && !!record.approval && typeof record.approval.required === "boolean" && (record.approval.outcome === "not_required" || record.approval.outcome === "approved" || record.approval.outcome === "denied") && (record.approval.actor === "policy" || record.approval.actor === "user") && typeof record.approval.automatic === "boolean" && !!record.result && (record.result.outcome === "success" || record.result.outcome === "failure" || record.result.outcome === "refused") && (record.result.duration_ms === null || typeof record.result.duration_ms === "number") && (record.result.error === null || typeof record.result.error === "string") && !!record.target && (record.target.path === null || typeof record.target.path === "string") && (record.target.before_sha256 === null || typeof record.target.before_sha256 === "string") && (record.target.after_sha256 === null || typeof record.target.after_sha256 === "string");
}
function csvCell(value) {
  const text = typeof value === "string" ? value : JSON.stringify(value);
  const normalized2 = text ?? "";
  return /[",\r\n]/u.test(normalized2) ? `"${normalized2.replaceAll('"', '""')}"` : normalized2;
}
...
`;
  ...
  records(limit = DEFAULT_MAX_RECORDS, filters = {}) {
    ...
    const text = import_node_fs.default.readFileSync(this.filePath, "utf8");
...
function assertSyntheticWorkspaceMarker(currentWorkspace) {
  ...
  const marker = import_node_path2.default.join(current, SYNTHETIC_WORKSPACE_MARKER);
  let markerStat;
  try {
    markerStat = import_node_fs2.default.lstatSync(marker);
  } catch {
...
function normalizeRunCommand(command) {
  ...
  if (isReadXlsx) {
    ...
    const reportPaths = normalized2.length > 1 ? normalized2.filter((word) => !(import_node_path3.default.win32.dirname(word) === "." && import_node_path3.default.win32.basename(word).toLowerCase() === "\u96C6\u8A08\u53F0\u5E33.xlsx")) : normalized2;
...
function snapshotKey(abs, ctx) {
  return `${ctx.runId ?? "default"}:${abs}`;
}
function recordFileSnapshot(abs, ctx, before, existedBefore, after) {
...
async function getFilePrecondition(p, ctx) {
...
function normalizeWorkspaceGlob(pattern) {
  ...
  if (import_node_path3.default.isAbsolute(normalized2) || /^[A-Za-z]:/.test(normalized2) || normalized2.split("/").includes("..")) {
    ...
    async run(args, ctx) {
      ...
      if (args.recursive === false) {
        ...
        await walk(base, (f) => {
          ...
          if (!re || re.test(import_node_path3.default.basename(f))) out.push(import_node_path3.default.relative(ctx.workspace, f).replaceAll("\\", "/"));
        });
      }
    ...
    async run(args, ctx) {
      ...
      if (patterns.length > 0) {
        ...
        await walk(ctx.workspace, (file) => {
          const relative = import_node_path3.default.relative(ctx.workspace, file).replaceAll("\\", "/");
...
`;
        ...
        if (import_node_path3.default.extname(file.relative).toLowerCase() === ".xlsx") {
...
async function captureFileBinding(def, args, ctx) {
...
async function approvalPreconditionChanged(binding, ctx) {
...
function assertExternalBoundaryBeforeRequest(cfg, ctx, io) {
  if (cfg.provider !== "external-openai") return;
  try {
...
function warningResult(message, messages, io) {
...
var SyntheticBusinessScreen = class {
  ...
  render() {
    ...
    return {
      ...
      html: renderScreenHtml(snapshot2),
      width: this.width,
      height: this.height
    };
  }
  ...
  stateFingerprint() {
    ...
    return import_node_crypto4.default.createHash("sha256").update(JSON.stringify({ revision: this.revision, visible }), "utf8").digest("hex");
  }
  openCompany(companyId) {
    const normalized2 = typeof companyId === "string" ? companyId.trim() : "";
    if (!normalized2) throw new ComputerUseActionError("invalid_company_id", "company_id \u306F\u7A7A\u306B\u3067\u304D\u307E\u305B\u3093");
    if (this.openedCompanyId !== null) {
      throw new ComputerUseActionError("duplicate_action", "\u3053\u306E\u753B\u9762\u3067\u306F open_company \u306F\u65E2\u306B\u5B9F\u884C\u6E08\u307F\u3067\u3059");
    }
    if (!this.companyById.has(normalized2)) {
      throw new ComputerUseActionError("company_not_visible", "\u6307\u5B9A\u3055\u308C\u305F\u4F1A\u793E\u30AB\u30FC\u30C9\u306F\u73FE\u5728\u306E\u753B\u9762\u306B\u8868\u793A\u3055\u308C\u3066\u3044\u307E\u305B\u3093");
    }
    if (normalized2 !== this.targetCompanyId) {
      throw new ComputerUseActionError("wrong_target", "\u753B\u50CF\u3067\u9078\u629E\u3055\u308C\u305F\u4F1A\u793E\u30AB\u30FC\u30C9\u3068\u4E00\u81F4\u3057\u307E\u305B\u3093");
    }
    const beforeRevision = this.revision;
    this.openedCompanyId = normalized2;
    this.revision++;
    return {
      action: "open_company",
      changed: true,
      companyId: normalized2,
      beforeRevision,
...
function renderScreenHtml(snapshot2) {
  const cards = snapshot2.visibleCompanies.map((company) => {
  ...
  const opened = snapshot2.openedCompanyId ? snapshot2.visibleCompanies.find((company) => company.id === snapshot2.openedCompanyId) : void 0;
...
<title>${escapeHtml(snapshot2.title)}</title>
<style>
...
<body><main id="computer-use-demo" data-route="${snapshot2.route}" data-revision="${snapshot2.revision}" data-synthetic="true">
<header><div><h1>${escapeHtml(snapshot2.title)}</h1><p class="subtitle">Visual business card fixture</p></div><span class="route">${escapeHtml(snapshot2.route)}</span></header>
...
<div id="screen-status" class="status" role="status">${escapeHtml(snapshot2.statusText)}</div>
</main></body></html>`;
}
function parseColor(value, fallback) {
  const match2 = value.match(/^#([0-9a-f]{6})$/iu);
  if (!match2) return fallback;
  return [
    Number.parseInt(match2[1].slice(0, 2), 16),
    Number.parseInt(match2[1].slice(2, 4), 16),
    Number.parseInt(match2[1].slice(4, 6), 16),
    255
  ];
}
...
function encodeScreenPng(snapshot2) {
  const { width, height } = snapshot2;
  ...
  const margin = 32;
  const gap = 18;
  const detailHeight = snapshot2.openedCompanyId ? 170 : 0;
  const cardWidth = Math.floor((width - margin * 2 - gap) / 2);
  const cardHeight = snapshot2.openedCompanyId ? 132 : Math.max(150, Math.floor((height - 76 - margin * 2 - gap - 34) / 2));
  snapshot2.visibleCompanies.forEach((company, index) => {
    ...
    drawText(pixels, width, height, company.sector, x + 22, y + (snapshot2.openedCompanyId ? 87 : 94), 2, [100, 116, 139, 255]);
    if (needsAttention) {
  ...
  if (snapshot2.openedCompanyId) {
    ...
    const opened = snapshot2.visibleCompanies.find((company) => company.id === snapshot2.openedCompanyId);
    drawText(pixels, width, height, "COMPANY DETAIL", margin + 26, detailY + 18, 2, [252, 165, 165, 255]);
    drawText(pixels, width, height, opened?.id ?? snapshot2.openedCompanyId, margin + 26, detailY + 46, 3, [255, 255, 255, 255]);
...
// test/computer-use-safety.ts
var SECRET = "ISSUE53_SECRET_MUST_NEVER_PERSIST";
var IMAGE_SENTINEL = "ISSUE53_IMAGE_BYTES_MUST_NEVER_PERSIST";
var REASONING_SENTINEL = "ISSUE53_REASONING_MUST_NEVER_PERSIST";
function context(workspace, flags = {}) {
...
function targetId(screen) {
  return screen.targetCompanyId;
}
function nonTargetId(screen) {
  const candidate = screen.snapshot().visibleCompanies.find((company) => company.id !== targetId(screen));
  import_strict.default.ok(candidate, "fixture must contain a visible non-target company");
  return candidate.id;
}
async function execute(screen, io, input, options = {}) {
  ...
  return executeV2ToolCall(
    ...
    def,
    options.cfg ?? config(),
    context(import_node_os2.default.tmpdir()),
    io,
    options.beforeHooks ?? []
  );
}
async function testValidationAndHookOrder() {
  ...
  import_strict.default.equal(invalid.status, "failed");
  import_strict.default.equal(invalid.executed, false, "schema failures must not execute ToolDef.run");
  import_strict.default.equal(hookCalls, 0, "schema validation must precede hooks");
  import_strict.default.equal(recorded.approvals.length, 0, "schema failures must not request approval");
  import_strict.default.equal(recorded.events.length, 0, "schema failures must not create an audit event");
  let hookObservedByApproval = false;
  const successful = makeIO((question) => {
    import_strict.default.match(question, /open_company|実行を許可/u);
    import_strict.default.equal(hookObservedByApproval, true, "approval must follow every before-hook");
    return true;
  });
  const approved = await execute(screen, successful.io, { company_id: targetId(screen) }, {
    beforeHooks: [({ tool: tool2, args }) => {
      import_strict.default.equal(tool2, "host.open_company");
      import_strict.default.equal(args.company_id, targetId(screen));
      hookObservedByApproval = true;
    }],
    callId: "issue53-order-success"
  });
  import_strict.default.equal(approved.status, "succeeded");
  import_strict.default.equal(approved.executed, true);
  import_strict.default.equal(successful.approvals.length, 1);
  import_strict.default.equal(screen.isOpened, true);
  const types = eventTypes(successful.events);
  import_strict.default.ok(types.indexOf("tool.requested") >= 0);
  import_strict.default.ok(types.indexOf("approval.requested") > types.indexOf("tool.requested"));
  import_strict.default.ok(types.indexOf("approval.resolved") > types.indexOf("approval.requested"));
  import_strict.default.ok(types.indexOf("tool.started") > types.indexOf("approval.resolved"));
  import_strict.default.ok(types.indexOf("tool.succeeded") > types.indexOf("tool.started"));
  const terminal = successful.events.find((event) => event.type === "tool.succeeded");
  import_strict.default.ok(terminal?.audit, "terminal host event must carry redacted audit metadata");
  import_strict.default.equal(terminal?.audit?.arguments.sha256.length, 64);
}
async function testPermissionAndApprovalFailClosed() {
  ...
  import_strict.default.equal(denied.status, "denied");
  import_strict.default.equal(denied.executed, false);
  import_strict.default.equal(deniedIO.approvals.length, 0, "permission deny must precede approval");
  import_strict.default.equal(denyScreen.isOpened, false);
  import_strict.default.ok(deniedIO.events.some((event) => event.type === "tool.denied"));
  ...
  const hookDenied = await execute(hookScreen, hookIO.io, { company_id: targetId(hookScreen) }, {
    beforeHooks: [() => {
      throw new Error("issue53-hook-deny");
    }],
    callId: "issue53-hook-denied"
  });
  import_strict.default.equal(hookDenied.status, "denied");
  import_strict.default.equal(hookDenied.executed, false);
  import_strict.default.equal(hookIO.approvals.length, 0);
  import_strict.default.equal(hookScreen.isOpened, false);
  import_strict.default.ok(hookIO.events.find((event) => event.type === "tool.denied")?.error?.includes("issue53-hook-deny"));
  ...
  import_strict.default.equal(rejected.status, "denied");
  import_strict.default.equal(rejected.executed, false);
  import_strict.default.equal(rejectIO.approvals.length, 1);
  import_strict.default.equal(rejectIO.approvals[0].approved, false);
  import_strict.default.equal(rejectScreen.isOpened, false, "a rejected approval must not mutate the screen");
}
async function testPreconditionAndHardGuards() {
  ...
  import_strict.default.equal(stale.status, "failed");
  import_strict.default.equal(stale.executed, true, "the host reached ToolDef.run, where the hard precondition guard failed");
  import_strict.default.equal(opens, 0, "stale precondition must block the structured action");
  import_strict.default.match(stale.output, /画面状態が変わった|stale_screen/u);
  import_strict.default.ok(staleIO.events.some((event) => event.type === "tool.failed"));
  ...
  import_strict.default.equal(wrong.status, "failed");
  import_strict.default.equal(wrong.executed, true);
  import_strict.default.equal(wrongScreen.isOpened, false);
  import_strict.default.match(wrong.output, /一致しません|wrong_target/u);
  ...
  import_strict.default.equal(invisible.status, "failed");
  import_strict.default.equal(invisible.executed, true);
  import_strict.default.equal(invisibleScreen.isOpened, false);
  import_strict.default.match(invisible.output, /表示されていません|company_not_visible/u);
  ...
  import_strict.default.equal(first.status, "succeeded");
  ...
  import_strict.default.equal(second.status, "failed");
  import_strict.default.equal(second.executed, true);
  import_strict.default.equal(duplicateScreen.currentRevision, revision);
  import_strict.default.match(second.output, /既に実行済み|duplicate_action/u);
}
function writeSyntheticWorkspace(root) {
  const workspace = import_node_path6.default.join(root, "synthetic");
  import_node_fs4.default.mkdirSync(workspace, { recursive: true });
  import_node_fs4.default.writeFileSync(import_node_path6.default.join(workspace, SYNTHETIC_WORKSPACE_MARKER), JSON.stringify(SYNTHETIC_WORKSPACE_MARKER_EXPECTED), "utf8");
  return workspace;
}
async function testExternalBoundaryAndStateIsolation() {
  const root = import_node_fs4.default.mkdtempSync(import_node_path6.default.join(import_node_os2.default.tmpdir(), "misen-issue53-external-"));
  const configRoot = import_node_path6.default.join(root, "config");
  import_node_fs4.default.mkdirSync(configRoot, { recursive: true });
  const synthetic = writeSyntheticWorkspace(configRoot);
  const outside = import_node_path6.default.join(root, "outside");
  import_node_fs4.default.mkdirSync(outside, { recursive: true });
  import_node_fs4.default.writeFileSync(import_node_path6.default.join(outside, "state.json"), JSON.stringify({ messages: [{ content: SECRET }] }), "utf8");
  const cfg = config({
    ...
    configPath: import_node_path6.default.join(configRoot, "config.json")
  });
  ...
  try {
    ...
    import_strict.default.ok(outsideIO.events.some((event) => event.type === "run.warning" && event.metadata?.safetyBoundary === "external-synthetic-workspace"));
    import_strict.default.equal(outsideIO.events.filter((event) => event.type === "model.decision").length, 0);
    ...
    import_strict.default.equal(modelCalls, 1, "callback image must be rejected before a second external model request");
  } finally {
    if (previous === void 0) delete process.env.ISSUE53_EXTERNAL_KEY;
    else process.env.ISSUE53_EXTERNAL_KEY = previous;
    import_node_fs4.default.rmSync(root, { recursive: true, force: true });
  }
}
async function listenOllamaMock(response) {
  const requests = [];
  const server = import_node_http.default.createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      const request = { body, raw, authorization: typeof req.headers.authorization === "string" ? req.headers.authorization : null };
...
async function testEphemeralImageReasoningAndAuditRedaction() {
  const workspace = import_node_fs4.default.mkdtempSync(import_node_path6.default.join(import_node_os2.default.tmpdir(), "misen-issue53-image-"));
  ...
  try {
    const result = await runAgentTurnV2({
      ...
      beforeHooks: [({ tool: tool2 }) => {
        beforeHooks.push(tool2);
      }],
      afterToolObservation: async (observation) => {
        import_strict.default.equal(observation.toolName, "open_company");
        import_strict.default.equal(observation.status, "succeeded");
    ...
    import_strict.default.equal(result.aborted, false);
    import_strict.default.equal(result.reply, "Vision\u78BA\u8A8D\u304C\u5B8C\u4E86\u3057\u307E\u3057\u305F");
    import_strict.default.equal(screen.isOpened, true);
    import_strict.default.deepEqual(beforeHooks, ["host.open_company"]);
    import_strict.default.equal(recorded.approvals.length, 1);
    const persisted = JSON.stringify({ messages: result.messages, events: recorded.events });
    import_strict.default.equal(persisted.includes(IMAGE_SENTINEL), false);
    import_strict.default.equal(persisted.includes(REASONING_SENTINEL), false);
    import_strict.default.equal(persisted.includes(SECRET), false);
    import_strict.default.equal(persisted.includes(Buffer.from(imageBytes).toString("base64")), false);
    import_strict.default.ok(recorded.events.every((event) => event.type !== "model.decision" || !bodyContains(event.metadata, IMAGE_SENTINEL)));
    import_strict.default.ok(recorded.events.filter((event) => event.type === "tool.succeeded").every((event) => !bodyContains(event, IMAGE_SENTINEL)));
    import_strict.default.equal(mock.requests.length, 2);
    import_strict.default.equal(mock.requests[0].authorization, null, "Ollama loopback must not receive an API secret");
    import_strict.default.equal(mock.requests[0].raw.includes("image"), true);
    import_strict.default.equal(mock.requests[0].raw.includes(SECRET), false);
    import_strict.default.equal(mock.requests[1].raw.includes(SECRET), false);
    const terminal = recorded.events.find((event) => event.type === "tool.succeeded");
    import_strict.default.ok(terminal);
    const history = recorded.events;
    const audit = auditRecordFromOutcome({ sessionId: "issue53-session", runId: "issue53-run", event: terminal, history });
    import_strict.default.ok(audit);
    const auditDirectory = import_node_fs4.default.mkdtempSync(import_node_path6.default.join(import_node_os2.default.tmpdir(), "misen-issue53-audit-"));
    const auditLog = new AuditLog({ workspace, directory: auditDirectory });
    auditLog.initialize();
    auditLog.append(audit);
    const auditText = import_node_fs4.default.readFileSync(auditLog.filePath, "utf8");
    import_strict.default.equal(auditText.includes(IMAGE_SENTINEL), false);
    import_strict.default.equal(auditText.includes(REASONING_SENTINEL), false);
    import_strict.default.equal(auditText.includes(SECRET), false);
    import_strict.default.equal(auditText.includes(Buffer.from(imageBytes).toString("base64")), false);
    auditLog.close();
    import_node_fs4.default.rmSync(auditDirectory, { recursive: true, force: true });
  } finally {
    await new Promise((resolve) => mock.server.close(() => resolve()));
    import_node_fs4.default.rmSync(workspace, { recursive: true, force: true });
  }
}
async function main() {
