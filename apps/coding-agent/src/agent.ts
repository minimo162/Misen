...
function composeCopilotPrompt(mode: TurnMode, userInput: string, steps: string[], budget = 120000, history: { role: string; content: string }[] = [], allowArbitraryCommands = false, autoApproveCommand = false): string {
...
async function runCopilotTurn(opts: {
  ...
  for (let i = 0; i < maxIter; i++) {
    ...
    let raw: string
    try {
      raw = await backend.complete(composeCopilotPrompt('work', opts.userInput, steps, cfg.copilot?.maxPromptChars ?? 120000, history, policy.allowArbitraryCommands, policy.autoApproveCommand), io.signal)
