...
// Misen modification: explicit .js aliases are required by the ESM TypeScript build.
...
const AttachmentUI: FC = () => {
  ...
  const errorMessage = useAuiState((s) =>
    s.attachment.status.type === "incomplete" &&
    s.attachment.status.reason === "error"
      ? (s.attachment.status.message ?? "持ち込みに失敗しました")
      : undefined,
  );
            ...
            <TooltipTrigger asChild>
              <div
                ...
                aria-label={`${typeLabel} attachment${
                  isError ? "、持ち込み失敗" : isUploading ? "、持ち込み中" : ""
                }`}
              >
                <AttachmentThumb />
                {isUploading && (
